...
%        showName = {'L lane','R Lane','Straight'};
    ...
    fname_ext = '_f_14_ww_20_df_20.mat';
    disp(fname_ext);
    folds_action = containers.Map;
    for i = 1:size(actions,2)
        action = actions{i};
        load(['observations/' action fname_ext]);      
    ...
    if retrain_models
        for numiter = 1:5
            if use_fixed_split
                if exist(['observations/' fixed_split_name num2str(numiter) '.mat'],'file') == 2
                    load(['observations/' fixed_split_name num2str(numiter) '.mat']);
                else
                    use_fixed_split = false;
                end;
            end;
            ...
            parfor i = 1:size(actions,2)
                ...
                if use_fixed_split
                    ...
                    load_data = load(['observations/' action fname_ext]);
                    data = load_data.data;
                ...
                model.iotype = false;
                model = initializeHMMmodel(model.type,model.nstates,model.observationDimension,model.iotype);
                model.action = action;
                ...
                [model,ll] = hmmFit(model,train_data,model);
            ...
            save(['observations/learnedModel_' train_model_for '_hmm_' num2str(numiter) fname_ext],'learnedModels','testData','trainData','testObs','trainObs','folds_action');
            %{
            if separate_actions
                save(['observations/learnedModel_' num2str(numiter) fname_ext '.mat'],'learnedModels','testData','trainData');
            else
                save(['observations/learnedModel_combined_' num2str(numiter) fname_ext '.mat'],'learnedModels','testData','trainData');
            end;
            %}
        end;
    end;
    ...
    accuracy = []; 
        for numiter = 1:5
            load(['observations/learnedModel_' train_model_for '_hmm_' num2str(numiter) fname_ext]);
            %{
            if separate_actions
                load(['observations/learnedModel_' num2str(numiter) fname_ext '.mat']);
            else
                load(['observations/learnedModel_combined_' num2str(numiter) fname_ext '.mat']);
            end;
            %}
            prediction_local = [];
            true_label_local = [];
            ...
            for i = 1:size(actions,2)
                ...
                for j = 1:size(test_data,2)
                    ...
                    prediction_probability = timeSeriesPrediction(learnedModels,test_data{j},actions);      
        ...
        savename = ['Figures/confMat_hmm_' fname_ext '_' num2str(THRESH) '.jpg'];
...
function prediction_probability = timeSeriesPrediction(learnedModels,test_data,actions)
    T = size(test_data,2);
